...
class SpiderLeg:
    ...
    def forwardKinematics(self, angles=None):
        ...
        phi1 = acos(round((self.FEMUR ** 2 + H ** 2 - self.TIBIA ** 2) / (2 * self.FEMUR * H), 4))
    ...
    def compute_angles(self, pos):
        ...
        new_angles = self.inverseKinematics(new_pos)
        joint_positions = self.forwardKinematics()

        if reverse_x:
            for i in range(4):
                joint_positions[i][0] *= -1

        return new_angles, joint_positions